...
class ProjectManagementGUI:
    ...
    def create_option_buttons(self):
        options = [
            ...
            ("View All Employees", self.view_all_employees),  
            ("View All Projects", self.view_all_projects),
            ("View All Tasks", self.view_all_tasks),
            ("Exit", self.root.quit)
        ]
    ...
    def view_all_employees(self):
        try:
            ...
            data = [(e.get_id(), e.get_name(), e.get_designation(), e.get_gender(), e.get_salary(), e.get_project_id()) for e in employees]
            self.display_table(["ID", "Name", "Designation", "Gender", "Salary", "ProjectID"], data,
    ...
    def view_all_projects(self):
        try:
            ...
            data = [(p.get_id(), p.get_name(), p.get_description(), p.get_start_date(), p.get_status()) for p in projects]
            self.display_table(["ID", "Name", "Description", "StartDate", "Status"], data,
    ...
    def view_all_tasks(self):
        try:
            ...
            data = [(t.get_task_id(), t.get_task_name(), t.get_employee_id(), t.get_project_id(), t.get_status()) for t in tasks]
            self.display_table(["ID", "Name", "ProjectID", "EmpID", "Status"], data,
                               ["Task ID", "Name", "Project ID", "Employee ID", "Status"])
        except Exception as e:
            messagebox.showerror("Error", str(e))
